...
import { businessDateParts, businessMonthStart, dashboardPeriod } from './dashboardPeriod.js';
...
export class DashboardRepository implements IDashboardRepository {
  async getDashboardRawData({ userId, year, month, dateFrom, dateTo }: IGetDashboardRawDataFilters) {
    ...
    const historyEnd = businessMonthStart(today.year, today.month - 1, today.day + 1);
    const historyStart = businessMonthStart(today.year, today.month - 12);
    ...
    const [rows, historyRows, newCustomers] = await prisma.$transaction([
      ...
      prisma.appointment.findMany({
        where: { userId, deletedAt: null, appointmentDate: { gte: historyStart, lt: historyEnd } },
        select: appointmentSelect,
        orderBy: [{ appointmentDate: 'desc' }, { id: 'asc' }],
      }),
      prisma.customer.count({ where: { userId, deletedAt: null, createdAt: { gte: start, lt: end } } }),
    ]);
    const normalize = (source: typeof rows): DashboardAppointment[] => source.map(({ customer, financeEntries, ...row }) => ({
    ...
    return { appointments: normalized.filter(a => a.appointmentDate >= start),
      previousAppointments: normalized.filter(a => a.appointmentDate < start),
      monthToDateAppointments: normalize(historyRows), newCustomers };
  }
}
